...
def find_magnus(message):
    ...
    '''
    if re.search("sp.+?ce|elon musk|rocket|tesla|new space conspiracy|artificial intelligence|coffe|complexity|hard sci-fi|anti-authoritarian|anarchist|nanofabricator|mars|minion|puppet|democracy.+?failed|moonbase|hex based|capitalism.+?doesn't work|capitalism.+?failed|capitalist|cynicism|hero system|complex|conspiracy",message.content.lower()): #Reacts to space
        return True
    else:
        return False
